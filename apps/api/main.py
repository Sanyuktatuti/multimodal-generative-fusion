...
# Lazy import routes to avoid startup issues
try:
    from apps.api.routes.planning import router as planning_router
    app.include_router(planning_router)
    print("✅ Planning router loaded successfully")
except Exception as e:
    print(f"❌ Failed to load planning router: {e}")

try:
    from apps.api.routes import envgen as envgen_router
    app.include_router(envgen_router.router)
    print("✅ EnvGen router loaded successfully")
except Exception as e:
    print(f"❌ Failed to load envgen router: {e}")
