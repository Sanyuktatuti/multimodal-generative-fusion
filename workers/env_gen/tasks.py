...
app = Celery("env_gen", broker=broker_url, backend=backend_url)
...
S3_BUCKET = os.getenv("S3_BUCKET")  # e.g., s3://bucket
...
def run_env_cloud(self, prompt: str) -> dict:
    job_id = f"envgen-{uuid.uuid4().hex[:8]}"
    payload = {"prompt": prompt, "out_bucket": S3_BUCKET, "job_id": job_id}
